...
from Features.Favorites.favorites_controller import FavoritesController # 💡 신규 임포트
from Features.Modify.modify_controller import ModifyController           # 💡 [신규] Modify 컨트롤러
...
class AppController(QObject):
    ...
    def __init__(self, _wgt_main):
        ...
        self.ctrl_favorites = FavoritesController(self.wgt_main.wgt_favorites_panel, self.obj_asset_manager)
        self.ctrl_modify = ModifyController(self.wgt_main, self.obj_asset_manager, self.obj_category_manager)  # 💡 [신규]
    ...
    def initConnections(self):
        ...
        # 💡 [추가] 즐겨찾기 패널 클릭 필터링
        self.ctrl_favorites.wgt_favorites_panel.wgt_category_panel.sig_category_clicked.connect(self.handleFavoritesFilter)

        # 💡 [추가] 즐겨찾기 뷰 컨트롤러에서 변경 신호를 받으면 트리 재구축 지시
        self.ctrl_view.sig_asset_favorite_toggled.connect(self.ctrl_favorites.handleRebuildFavoritesTree)
        self.ctrl_cache.sig_cache_loaded.connect(self.ctrl_favorites.handleRebuildFavoritesTree)

        # 💡 [추가] 뷰 컨트롤러에서 삭제되었다는 신호를 받으면 화면 갱신
        self.ctrl_view.sig_asset_deleted.connect(self.handleAssetDeleted)

        # 💡 [신규] Modify Asset 요청 신호 연결
        self.ctrl_view.sig_modify_requested.connect(self.ctrl_modify.handleOpenModifyWindow)
        # 💡 [신규] 수정 완료 시 카테고리 트리 재구축 + 그리드뷰 갱신
        self.ctrl_modify.sig_asset_modified.connect(self.handleRebuildCategoryTree)
        self.ctrl_modify.sig_asset_modified.connect(lambda: self.ctrl_view.refreshGridView())
    ...
    def handleFavoritesFilter(self, _list_category_path):
        """즐겨찾기 패널의 카테고리 클릭 보고를 받고 필터링된 화면 갱신을 지시합니다."""
        print(f"💖👔 [AppController] 즐겨찾기 '{_list_category_path}' 필터링 지시 접수!")
        
        # 1. 먼저 전체 에셋 중 해당 카테고리로 필터링을 한 뒤
        list_filtered = self.obj_asset_manager.getFilteredAssets(_list_category_path)
        
        # 2. 거기서 즐겨찾기된 애들만 남깁니다
        list_favorites_only = [asset for asset in list_filtered if asset.bool_favorite]
        
        print(f"🔍 [AppController] 즐겨찾기 검색 완료: 총 {len(list_favorites_only)}개 발견됨.")
        self.ctrl_view.refreshGridView(list_favorites_only)

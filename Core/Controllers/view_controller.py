...
from Features.ThumbnailView.asset_context_menu import AssetContextMenu

class ViewController(QObject):
    sig_asset_deleted = Signal() # 💡 삭제 완료를 메인 컨트롤러에 알리는 신호
    ...
    def handleGoToFiles(self, _str_path):
        """우클릭 'Go to Files' 메뉴가 눌렸을 때 파일이 있는 폴더를 엽니다."""
        print(f"📂 [ViewController] Go to Files 요청: {_str_path}")
        self.obj_asset_manager.openAssetFolder(_str_path)
    ...
    def handleDeleteList(self, _str_id):
        ...
        if bool_deleted:
            # 2. 삭제 성공 시, 화면 갱신을 위해 메인 컨트롤러에 신호 전송

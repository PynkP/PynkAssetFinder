# Features/Category/category_panel.py
...
class CategoryPanel(QWidget):
    ...
    # 클릭 시 컨트롤러에게 전체 경로(리스트)를 파악해서 전달할 시그널
    sig_category_clicked = Signal(list)
    ...
    def initUI(self):
        ...
        self.wgt_tree.setIndentation(12) # 💡 뎁스 간격 줄이기 (기본 20)
        ...
        self.wgt_tree.setIndentation(12)
    ...
    def _onItemClicked(self, _item, _column):
        """
        아이템이 클릭되면, 부모를 거슬러 올라가며 전체 경로를 파악해 넘깁니다.
        """
        list_path = []
        current_item = _item
        while current_item is not None:
            str_name = current_item.data(0, Qt.UserRole)
            if str_name == "Root":
                break
            list_path.insert(0, str_name)
            current_item = current_item.parent()
            
        print(f"🖱️ [CategoryPanel] 카테고리 경로 클릭됨: {list_path}")
        self.sig_category_clicked.emit(list_path)

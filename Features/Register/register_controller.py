# Features/Register/register_controller.py
...
from Core.Utils.id_manager import IDManager                            # 💡 새 경로
from Features.Register.make_data_controller import MakeDataController  # 💡 새 경로

class RegisterController(QObject):
    # 등록 성공 시 UI 갱신을 위해 사장님(AppController)에게 보낼 신호
    ...
    def handleMakeDataProcess(self):
        ...
        dict_save_data = {
            "id": str_id,  # 💡 [수정됨] 데이터 무결성을 위해 내부에 ID 명시적 추가
            "semanticTags": {
                "name": str_asset_name,
    ...
    def handleLoadData(self):
        ...
        # 5. 사장님(AppController)에게 UI 갱신 요청
